...
import { Wrench, ShieldCheck, Zap, Clock } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFC] dark:bg-[#0F172A] text-[#0F172A] dark:text-white selection:bg-[#2563EB] selection:text-white relative">
      ...
      <section className="relative bg-[#0F172A] text-white pt-8 sm:pt-12 pb-16 sm:pb-24 px-3 sm:px-6 lg:px-8 overflow-hidden border-b border-slate-800">
        <div className="absolute top-0 right-1/4 w-96 h-96 bg-rose-600/25 rounded-full blur-[110px] pointer-events-none" />
        <div className="absolute bottom-0 left-10 w-80 h-80 bg-[#2563EB]/25 rounded-full blur-[100px] pointer-events-none" />
        <div className="absolute inset-0 opacity-20 bg-[radial-gradient(#F43F5E_1px,transparent_1px)] [background-size:20px_20px] pointer-events-none" />
        ...
        <div className="max-w-6xl mx-auto space-y-5 relative z-10">
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight">
            House Maids, Daily Cooks & Laundry in <br className="hidden sm:inline" />
            <span className="bg-gradient-to-r from-[#FDA4AF] via-[#FB7185] to-[#93C5FD] bg-clip-text text-transparent">
              {selectedLocality ? selectedLocality.name : selectedCity.name}
            </span>
          </h1>

          <p className="text-xs sm:text-base text-slate-300 max-w-2xl font-normal leading-relaxed">
            Verified domestic helpers, 24-hr laundry doorstep pickup, AC & RO servicing, and local packers & movers.
          </p>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 max-w-4xl pt-2">
            {[
              { label: "Background Verified", val: "Aadhar Checked", icon: ShieldCheck, color: "text-[#10B981]" },
              { label: "Laundry Turnaround", val: "24-hr Doorstep", icon: Clock, color: "text-[#0EA5E9]" },
              { label: "Pricing", val: "Standard Rates", icon: Zap, color: "text-[#F59E0B]" },
              { label: "Instant Connect", val: "Direct Call", icon: Wrench, color: "text-[#2563EB]" },
            ].map((st, i) => {
              const Icon = st.icon;
              return (
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-3 sm:px-6 lg:px-8 pt-8 pb-32 sm:pb-12 space-y-8">
        <section className="glass-panel p-4 sm:p-5 rounded-3xl flex flex-wrap items-center justify-between gap-3 shadow-sm">
          <div>
            <h3 className="text-base font-black text-[#0F172A] dark:text-white">Verified Local Service Providers</h3>
            <p className="text-xs text-[#64748B]">Experienced maids, cooks, laundrymen & repair technicians</p>
          </div>
          <button
