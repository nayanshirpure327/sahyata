...
import { Briefcase, Zap, ShieldCheck, DollarSign, TrendingUp } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFC] dark:bg-[#0F172A] text-[#0F172A] dark:text-white selection:bg-[#2563EB] selection:text-white relative">
      ...
      <section className="relative bg-[#0F172A] text-white pt-8 sm:pt-12 pb-16 sm:pb-24 px-3 sm:px-6 lg:px-8 overflow-hidden border-b border-slate-800">
        <div className="absolute top-0 right-1/4 w-96 h-96 bg-[#10B981]/25 rounded-full blur-[110px] pointer-events-none" />
        ...
        <div className="max-w-6xl mx-auto space-y-5 relative z-10">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="px-3.5 py-1.5 rounded-full glass-pill text-emerald-300 text-xs font-black uppercase tracking-wider flex items-center gap-1.5">
              <Briefcase className="w-3.5 h-3.5 text-[#10B981]" />
              Newcomer Jobs & Gigs Portal
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight">
            Jobs, Gigs & Hiring in <br className="hidden sm:inline" />
            <span className="bg-gradient-to-r from-[#6EE7B7] via-[#10B981] to-[#93C5FD] bg-clip-text text-transparent">
              {selectedLocality ? selectedLocality.name : selectedCity.name}
            </span>
          </h1>

          <p className="text-xs sm:text-base text-slate-300 max-w-2xl font-normal leading-relaxed">
            Start earning immediately in your new city. Freshers customer operations, home tutoring, store associates, and freelance gigs.
          </p>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 max-w-4xl pt-2">
            {[
              { label: "Immediate Hiring", val: "Freshers Welcome", icon: Zap, color: "text-[#10B981]" },
              { label: "Salary Range", val: "₹18k - ₹35k / mo", icon: DollarSign, color: "text-[#F59E0B]" },
              { label: "Shift Flexibility", val: "Day / Part-Time", icon: TrendingUp, color: "text-[#0EA5E9]" },
              { label: "Verified Employers", val: "100% Direct", icon: ShieldCheck, color: "text-[#2563EB]" },
            ].map((st, i) => {
              const Icon = st.icon;
              return (
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-3 sm:px-6 lg:px-8 pt-8 pb-32 sm:pb-12 space-y-8">
        <section className="glass-panel p-4 sm:p-5 rounded-3xl flex flex-wrap items-center justify-between gap-3 shadow-sm">
          <div>
            <h3 className="text-base font-black text-[#0F172A] dark:text-white">Active Job Openings</h3>
            <p className="text-xs text-[#64748B]">Apply directly with 1-click WhatsApp or Call to recruiters</p>
          </div>
          <button
            onClick={() => setPostModalOpen(true)}
            className="px-5 py-2 bg-[#F59E0B] text-black font-black rounded-xl text-xs shadow-md shadow-[#F59E0B]/25"
          >
            + Post Job / Gig Opening
          </button>
        </section>
